...
const BookingHistory = ({navigation}: any) => {
  ...
  function fetchPostData() {
    ...
    getPostRequests()
      .then((rest: any) => {
        {
          rest.success && (setPostData(rest.postrequests), setLoading(false));
        }
      })
      .catch(error => {
        if (error.response.status === 401) {
                        ...
                        item.request.isMakePayment === false
                          ? navigation.navigate('StripePayment', {
                              item: {
                                ...
                                amount: item.request.bookingId?.totalFare
                                  ? item.request.bookingId?.totalFare
                                  : 30,
                              },
                            })
                          ...
                          ? navigation.navigate('TrackShip', {
                              ...
                              dropAddress:
                                item?.request?.bookingId?.dropAddress,
                            })
                          : item?.request?.type === 'Land'
                      ...
                      handleTracking={() => {
                        item.isMakePayment === false
                          ...
                          ? navigation.navigate('TrackLand', {
                              ...
                              pickupAddress: item?.bookingId?.pickupAddress,
                              dropAddress: item?.bookingId?.dropAddress,
                              requestId: item.request.isCustomerRead,
                            })
                          : null;
                      }}
                    />
