...
const AddVehicle = ({ navigation }: any) => {
    ...
    const validForm = () => {
        ...
        if (type && name && color && model && licence && vImage && vLicence && vResidence) {
            getAllImagesUrlByPromise();
        }
    }
    const uploadDataToServer = (response: any) => {
        let insuranceValue = "";
            ...
            {isLoading ? <MyLoader /> :
                <View style={styles.container}>
                    <VehicleInputRow
                        ...
                        validMessage={"Vehicle Type is Required"}
                    ...
                    <VehicleImageRow
                        ...
                        title={Object.keys(vehicleImage).length === 0 ? "VEHICLE IMAGE" : vehicleImage.name}
                        onPress={() => { imagePicker("VEHICLEIMAGE") }} svgImage={ImagePickerSvg} />
                    <VehicleImageRow
                        title={Object.keys(vehicleInsurance).length === 0 ? "VEHICLE INSURANCE" : vehicleInsurance.name}
                        onPress={() => { imagePicker("VEHICLEINSURANCE") }}
                        svgImage={ImagePickerSvg} />
                    <VehicleImageRow
                        isValid={isVehicleLicence}
                        validMessage={"Vehicle Licence is Required"}
                        title={Object.keys(vehicleLicence).length === 0 ? "VEHICLE LICENCE" : vehicleLicence.name}
                        onPress={() => { imagePicker("VEHICLELICENCE") }}
                        svgImage={ImagePickerSvg} />
                    <VehicleImageRow
                        isValid={isVehicleResidence}
                        validMessage={"Vehicle Residence is Required"}
                        title={Object.keys(vehicleResidence).length === 0 ? "VEHICLE RESIDENCE" : vehicleResidence.name}
                        onPress={() => { imagePicker("VEHICLERESIDENCE") }}
                        svgImage={ImagePickerSvg} />
                    <Button
                        onPress={validForm}
